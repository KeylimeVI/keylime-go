...
func Map[T comparable, U comparable](collection Set[T], iteratee func(item T) U) Set[U] {
	result := SetOf[U]()
...
func FlatMap[T comparable, U comparable, S ~[]U](collection Set[T], iteratee func(item T) S) Set[U] {
	result := SetOf[U]()
	for item := range collection {
